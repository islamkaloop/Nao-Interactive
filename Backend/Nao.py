...
from Speech import VoicRecog
from ArabicSpeech import ArVoicRecog
from seqspeech import seqVoicRecog
from seqArabicSpeech import seqArabicVoicRecog
import qi
import sys

ROBOT_IP = '192.168.43.148'

class generate (object):

    def __init__(self,language,app):
        self.app=app
        self.NAO_IP = ROBOT_IP
        parser = OptionParser()
        parser.add_option(
        ...
        self.myBroker = ALBroker("myBroker", "0.0.0.0",  # Listen to anyone
                                 ...
                                 self.pip,               # parent broker ip
                                 self.pport)             # parent broker port
        # Create ALProxy
        ...
        self.voice_reco = ALProxy("ALSpeechRecognition")
        if(language=="English"):
            self.tts.setLanguage("English")
            self.say("Hi there, let's start")
        else:
            self.tts.setLanguage("Arabic")
            self.say("أهلاً و سهلاً، هيا نبدأ")
        self.modelNum=0
    ...
    def voiceReco(self):
        try:
            if(self.tts.getLanguage() == 'English'):
                ...
                self.say("Ok, you can order me by voice command now")
                VoicRecog(self.app,self)
            else:
                # Initialize qi framework.
                self.say("حسناً، أنا مستعد لإستقبال أوامرك الاَن")
                ArVoicRecog(self.app,self)
        except RuntimeError:
            print ("Can't connect to Naoqi at ip \"" + self.pip + "\" on port " + str(self.pport) +".\n"
                "Please check your script arguments. Run with -h option for help.")
            sys.exit(1)
        #Voic_Recog.run()

    
    def seqVoiceReco(self):
        try:
            if(self.tts.getLanguage() == 'English'):
                # Initialize qi framework.
                Voic_Recog = seqVoicRecog(self.app,self)
                
            else:
                # Initialize qi framework.
                Voic_Recog = seqArabicVoicRecog(self.app,self)
    ...
    def stand(self):
        if(self.posture.getPosture()=="StandInit"):
            if(self.tts.getLanguage()=="English"):
                self.say("I am already standing")
            else:
                self.say("أنا بالفعل واقف")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("Ok, I will stand up now")
            else:
                self.say("حسناً، سوف أقف حالاً")
    ...
    def sit(self):
        if(self.posture.getPosture()=="Sit"):
            if(self.tts.getLanguage()=="English"):
                ...
                self.say("أنا بالفعل جالس")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("Ok, I will sit down now")
            else:
                self.say("حسناً، سوف أجلس حالاً")
            self.posture.goToPosture("Sit", 1.0)

    def crouch(self):
        if(self.posture.getPosture()=="Crouch"):
            if(self.tts.getLanguage()=="English"):
                self.say("I am already Crouch")
            else:
                self.say("أنا بالفعل منحنى")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("Ok, I will Crouch now")
            else:
                self.say("حسناً، سوف أنحنى حالاً")
            self.posture.goToPosture("Crouch", 1.0)

    def LyingBelly(self):
        if(self.posture.getPosture()=="LyingBelly"):
            if(self.tts.getLanguage()=="English"):
                self.say("I am already lying on belly")
            else:
                self.say("أنا بالفعل راقد على صدرى")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("Ok, I will lie on belly now")
            else:
                self.say("حسناً، سوف أرقد على صدرى حالاً")
        self.posture.goToPosture("LyingBelly", 1.0)

    def LyingBack(self):
        if(self.posture.getPosture()=="LyingBack"):
            if(self.tts.getLanguage()=="English"):
                self.say("I am already lying on back")
            else:
                self.say("أنا بالفعل راقد على ظهرى")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("Ok, I will lie on back now")
            else:
                self.say("حسناً، سوف أرقد على ظهرى حالاً")
    ...
    def OpenHand(self, handName):
        if(handName == "LHand"):
            if(self.tts.getLanguage()=="English"):
                self.say("I will open my left hand")
            else:
                self.say("حسناً، سوف افتح يدى اليسرى حالاً")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("I will open my right hand")
            else:
                self.say("حسناً، سوف افتح يدى اليمنى حالاً")
        self.motion.openHand(handName)

    def CloseHand(self, handName):
        if(handName == "LHand"):
            if(self.tts.getLanguage()=="English"):
                self.say("I will close my left hand")
            else:
                self.say("حسناً، سوف اغلق يدى اليسرى حالاً")
        else:
            if(self.tts.getLanguage()=="English"):
                self.say("I will close my right hand")
            else:
                self.say("حسناً، سوف اغلق يدى اليمنى حالاً")
    ...
    def walk(self, x, y):
        if ~(x==0 and y==0):
            if(x>0):
                if(self.tts.getLanguage()=="English"):
                    self.say("Ok, I will move forward now")
                else:
                    self.say("حسناً، سوف أتحرك للأمام حالاً")
            else:
                if(self.tts.getLanguage()=="English"):
                    self.say("Ok, I will move backward now")
                else:
                    self.say("حسناً، سوف أتحرك للخلف حالاً")
            rad = math.radians(0)
    ...
    def turn(self, theta):
        if ~(theta == 0):
            if(theta>0):
                if(self.tts.getLanguage()=="English"):
                    self.say("Ok, I will turn left now")
                else:
                    self.say("حسناً، سوف اتجه لليسار حالاً")
            else:
                if(self.tts.getLanguage()=="English"):
                    self.say("Ok, I will turn right now")
                else:
                    self.say("حسناً، سوف اتجه لليمين حالاً")
            rad = math.radians(theta)
            self.motion.moveTo(0 / 100.0, 0 / 100.0, rad)

    def stop(self):
        if(self.tts.getLanguage()=="English"):
            self.say("Ok, I will stop soon")
        else:
            self.say("حسناً، سوف أقف حالاً")
    ...
    def taiChi(self):
        if(self.tts.getLanguage()=="English"):
            self.say("You want me to do taiChi, Ok lets start")
        else:
            self.say("أنت تريدنى أن اؤدى رقصة التاى تشى، حسناً لنبدأ")
        names = list()
        times = list()
        keys = list()
    ...
    def wave(self):
        ...
        if(self.tts.getLanguage()=="English"):
            self.say("Hello My friend") 
        else:
            self.say("أهلاً بصديقى المميز")
    ...
    def wipeForehead(self):
        ...
        if(self.tts.getLanguage()=="English"):
            self.say("Ohh, I am so tired")
        else:
            self.say("اوه، أنا مجهد جداً")
    ...
    def blowkiss(self):
        ...
        if(self.tts.getLanguage()=="English"):
            self.say("for my great friend")
        else:
            self.say("لاجل صديقى المميز")

        self.motion.angleInterpolation(names, keys, times, True)

    def macrenadance(self):
        if(self.tts.getLanguage()=="English"):
            self.say("You want me to do macrena dance, Ok lets start")
        else:
            self.say("أنت تريدينى ان أؤدى رقصة الماكارينا،حسناُ لنبدأ")
    ...
    def pushups(self):
        if(self.tts.getLanguage()=="English"):
            self.say("let's do some exercise")
        else:
            self.say("هيا نؤدى بعض التمارين")
        names = list()
        times = list()
        keys = list()
    ...
    def bow(self):
        if(self.tts.getLanguage()=="English"):
            self.say("My great appreciation")
        else:
            self.say("كل تقديرى وإحترامى")
        names = list()
        times = list()
        keys = list()
    ...
    def blink(self):
        if(self.tts.getLanguage()=="English"):
            self.say("Ok, I will blink my eyes")
        else:
            self.say("حسناً، سوف اجعل عيني تومض حالاً")            
    ...
    def say(self, text, speed=20, shape=30):
    ...
    def playFile(self):
        self.aup.playFile("MACARENA MP3 SONG.wav")
